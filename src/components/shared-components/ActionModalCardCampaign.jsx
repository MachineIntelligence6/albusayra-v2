...
const ActionModalCardCampaign = ({ onClose, rowIDOfCampaign }) => {
  const {
    register,
    handleSubmit,
  ...
  const filteredStatusOptions = useMemo(() => {
    if (pathname.includes("shortlisted-applicants")) {
      ...
      return statusOptions.filter(
        (status) => status.value === 7 || status.value === 15
      );
    } else if (pathname.includes("hold")) {
      return statusOptions.filter(
        (status) => status.value !== 6 && status.value !== 15
      );
    } else if (pathname.includes("not-qualified")) {
      return statusOptions.filter(
        (status) => status.value !== 9 && status.value !== 15
      ); // Remove "Not Qualified" and 15
    } else if (pathname.includes("incomplete-profile")) {
      return statusOptions.filter(
        (status) => status.value === 6 || status.value === 9
      );
    ...
    // Remove 15 from the default path
    return statusOptions.filter((status) => status.value !== 15);
  }, [pathname]);
