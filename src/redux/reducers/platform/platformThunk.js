...
// export const PlatformCreate = createAsyncThunk(
//   "companies/PlatformCreate",
//   async (data, { rejectWithValue }) => {
//     try {
//       const response = await axios.post(
//         `${Api.API_URL}${Api?.endpoints?.PlatformCreate}`,
//         data,
//         Api.multiPartConfig()
//       );
...
export const PlatformUpdate = createAsyncThunk(
  "platform/PlatformUpdate",
  async (data, { rejectWithValue }) => {
    try {
      const res = await axios.post(
        `${Api.API_URL}${Api.endpoints.PlatformUpdate}`,
        data,
        Api.multiPartConfig()
      );
...
export const PlatformDelete = createAsyncThunk(
  ...
  async (data, { rejectWithValue }) => {
    try {
      const res = await axios.post(
        `${Api.API_URL}${Api.endpoints.PlatformDelete}`,
        data,
        Api.config()
      );
...
// export const PlatformGetByStatus = createAsyncThunk(
//   "platform/PlatformGetByStatus",
//   async ({
//     page,
//     pageLength,
//     filter,
...
//     let response;
//     try {
//       let url = `${Api.API_URL}${Api.endpoints.PlatformGetByStatus}?page=${page}&pageLength=${pageLength}&filter=${filter}&filter2=${filter2}&filter3=${filter3}&desc=${desc}&orderBy=${orderBy}&parentId=${parentId}&SubParentId=${SubParentId}&SubParentId1=${SubParentId1}&SubParentId2=${SubParentId2}`;
  ...
  async (params, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(
        `${Api.API_URL}${Api?.endpoints?.PlatformGetByStatus}?${queryString}`,
        Api.config()
      );
      return response?.data?.result;
    } catch (error) {
...
export const PlatformGetById = createAsyncThunk(
  "platform/PlatformGetById",
  async ({ id }, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        `${Api.API_URL}${Api.endpoints.PlatformGetById}?id=${id}`,
        Api.config()
      );
